...
const StudentsManagement = () => {
  ...
  const handleViewTeachers = (student: User) => {
    ...
    const studentBookings = bookings.filter(
      (booking: { id: string; studentId: string }) =>
    ...
    const teacherIds = [
      ...new Set(
        studentBookings.map(
          (booking: { id: string; teacherId: string }) => booking.teacherId
        )
      ),
    ];
    ...
    const teachersList = users.filter(
      (user: User) => user.role === "teacher" && teacherIds.includes(user.id)
    );
  ...
  const handleDeleteStudent = () => {
    ...
    const updatedUsers = users.filter(
      (user: User) => user.id !== selectedStudent.id
    );
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-green/10 p-4 rounded-lg text-center">
                ...
                <p className="text-2xl font-bold text-green">
                  {students.filter((t) => t.isActive !== false).length}
                </p>
              </div>
