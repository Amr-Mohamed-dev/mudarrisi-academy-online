...
import { UserX, UserCheck, Eye, ArrowLeft } from "lucide-react";
...
const StudentsManagement = () => {
  ...
  const loadStudents = async () => {
    try {
      setIsLoading(true);
      const allUsers = await usersService.getByRole("STUDENT");
  ...
  const handleViewTeachers = async (student: User) => {
    setSelectedStudent(student);
    try {
      // جلب جميع الحجوزات لهذا الطالب
      const studentBookings = await bookingsService.getFiltered({
        studentId: student.id,
      });

      // الحصول على معرفات المدرسين الفريدة
      const teacherIds = [
        ...new Set(studentBookings.map((booking) => booking.teacherId)),
      ];

      // جلب بيانات المدرسين
      const allUsers = await usersService.getAll();
      const teachersList = allUsers.filter(
        (user) => user.role === "TEACHER" && teacherIds.includes(user.id)
      );
      ...
      addToast({
        title: "خطأ",
        message: "حدث خطأ في جلب مدرسي الطالب",
        type: "error",
      });
    }
  };
  ...
  const handleToggleStatus = async (student: User) => {
    try {
      const updatedStudent = await usersService.update(student.id, {
      ...
      if (updatedStudent) {
        ...
        addToast({
          title: "تم",
          message: `تم ${student.isActive ? "إيقاف" : "تنشيط"} الطالب بنجاح`,
          type: "success",
        });
      }
    } catch (error) {
  ...
  const handleDeleteStudent = async () => {
    ...
    try {
      await usersService.delete(selectedStudent.id);

      setStudents((prevStudents) =>
        prevStudents.filter((s) => s.id !== selectedStudent.id)
      );
      setFilteredStudents((prevFiltered) =>
        prevFiltered.filter((s) => s.id !== selectedStudent.id)
      );
              ...
              currentStudents.map((student) => (
                <TableRow key={student.id}>
                  ...
                  <TableCell>
                    <Badge
                      variant={
                        student.isActive === false ? "destructive" : "default"
                      }>
                        ...
                        {studentTeachers.length > 0 ? (
                          ...
                          <div className="text-center py-10">
                            <p className="text-muted-foreground">
                              لا يوجد مدرسين مرتبطين بهذا الطالب
                            </p>
                          </div>
                        )}
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <Button
                        variant={
                          student.isActive === false ? "default" : "outline"
                        }
                        size="sm"
