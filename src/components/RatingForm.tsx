...
const RatingForm = ({
    ...
    const handleSubmit = () => {
        if (rating === 0) {
            addToast({
                title: "الرجاء اختيار تقييم من 1 إلى 5 نجوم",
        ...
        const newRating = {
            ...
            raterId:
                raterRole === "student"
                    ? JSON.parse(localStorage.getItem("bookings") || "[]").find(
        ...
        notifications.push({
            ...
            message: `قام ${newRating.raterName} بتقييمك بـ ${rating} نجوم${
                comment ? ` وكتب: "${comment}"` : ""
            }`,
            read: false,
            createdAt: new Date().toISOString(),
        });
        ...
        <Card className="border shadow-sm">
            <CardHeader>
                <CardTitle className="text-lg">
                    {raterRole === "student" ? "تقييم المدرس" : "تقييم الطالب"}:{" "}
                    {targetName}
                </CardTitle>
            </CardHeader>
